...
@Injectable()
export class AdminSetReceptionMethodEnabledUseCase {
  constructor(
  ...
  async execute(input: { code: string; enabled: boolean }): Promise<ReceptionMethodCatalogReadModel> {
    ...
    await this.catalogsCommand.setReceptionMethodEnabled({ code, enabled: input.enabled });
